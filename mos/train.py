...
from mos.models import Estimator
from mos.pESA_models import hEstimator
from data_loading.data_loading import get_dataset_train_val_test
from mos.config import (
...
def main():
    ...
    if check_is_node_level_dataset(dataset):
        assert apply_attention_on == "node", "mos is not currently supported for node-level tasks!"

    if dataset == "ocp":
        assert apply_attention_on == "node", "mos is not currently supported for OCP!"
    ...
    # OCP branch
        ...
        from mos.models_OCP import Estimator as Estimator_OCP
